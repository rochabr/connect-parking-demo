...
let payoutsElement = null; // payouts component element
...
async function loadAccounts() {
  ...
  if (!selectedAccountId && accounts[0]?.id) {
    ...
    await initPayoutsForAccount(selectedAccountId);
  }
}
...
function renderAccounts() {
  ...
  accounts.forEach(a => {
    ...
    if (el) el.addEventListener('change', async () => {
      ...
      await initPayoutsForAccount(a.id);
    });
  });
...
/* ----------------- Connect.js wiring ----------------- */
async function initPayoutsForAccount(accountId) {
  try {
    hideError();

    // Tear down any existing component/instance
    if (payoutsElement) { try { payoutsElement.remove(); } catch {} payoutsElement = null; }
...
connectInstance = await loadConnectAndInitialize({
  ...
  appearance: {
    ...
    payoutsElement = connectInstance.create('payouts');
    containerEl.innerHTML = '';
    containerEl.appendChild(payoutsElement);
  } catch (e) {
...
refreshAccountsBtn.addEventListener('click', async () => {
  ...
  if (prev && accounts.find(a => a.id === prev)) {
    ...
    await initPayoutsForAccount(prev);
  }
});
